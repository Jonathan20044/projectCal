...
function checkCollision() {
  ...
  // dino hit the ground
  if (dinoPos_Y + hitboxHeight >= canvas.height - 150) {
    velocity = 0;
    dinoPos_Y = canvas.height - 150 - hitboxHeight;
    setGameOver();
  }
  ...
  if (
    ...
    dinoPos_X < bones[bonePtr].x + 80 &&
    (dinoPos_Y <= bones[bonePtr].y + 400 ||
      dinoPos_Y + hitboxHeight >= bones[bonePtr].y + 400 + flyGap)
  ) {
    setGameOver();
  }
}

function generateInitialBones() {
  bones = [];
  bonePtr = 0;
...
function update() {
  ...
  if (start) {
    ...
    if (!gameOver) {
      ...
      if (dinoPos_X >= bones[bonePtr].x + 80) {
        score++;
